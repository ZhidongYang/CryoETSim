...
class TEM:
    ...
    def add_detector_noise(self, snr):
        ...
        mics = (mics - mics.min()) / (mics.max() - mics.min())

        # Adding readout noise with Gaussian distribution to every micrograph
        # Re-edited by Zhidong Yang, with additional Poisson noise
        # (not adopted now, this will be replaced by generative noise).
        for i in range(mics.shape[2]):
            ...
            # lam = snr**2
            # # lam = sg_fg / 10
            # mic_poisson = np.clip(np.random.poisson(lam * mic)/lam, 0, 1)  # Re-edited by Zhidong Yang
            # mics[:, :, i] = mic
